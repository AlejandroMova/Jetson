...
Worker thread para grabación automática de clips de video en QA mode. `notify_detection(N)` es llamado directamente desde los probes (sin Redis). Estado: IDLE → RECORDING → IDLE. Graba `tiled.mp4` (desde `push_tiled_frame()`, llamado por MjpegServer) y `<camera_id>.mp4` full-res (desde `push_camera_frame()`, llamado por Probe A). Cooldown: 10 s sin detecciones. Duración mínima: 5 s (clips más cortos se descartan). Duración máxima: 5 min. Auto-prune: elimina clips más antiguos cuando total > 10 GB. Publica `nx:qa:recording_active` y `nx:qa:recording_info` a Redis. Directorio `/nx_tech/recordings/<YYYYMMDD_HHMMSS>/`: `thumbnail.jpg`, `metadata.json`, `tiled.mp4`, `<cam_id>.mp4`. **Importante:** `_max_people` NO se resetea en `_start()` (se preserva el conteo que disparó la grabación); se resetea en `_reset_writers()` tras cerrar el clip — de lo contrario todos los clips quedan con `max_people: 0`.
