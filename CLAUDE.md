...
- **Matching:** max-similitud coseno ≥ 0.85 (`SIMILARITY_THRESHOLD`) sobre **galería de hasta 10 embeddings** por persona (`GALLERY_MAX_SIZE`, configurable por cliente vía `reid_gallery_size`). `ReIdManager` — O(N×K), vectorizable con numpy. Nuevos ángulos se añaden a la galería solo si son suficientemente distintos a los existentes (0.71 ≤ sim < 0.95, `GALLERY_DIVERSITY_THRESHOLD_MIN/MAX`); cuando la galería está llena se reemplaza el miembro menos informativo.
  ...
  - `GALLERY_DIVERSITY_THRESHOLD_MAX` subido de 0.85 a 0.95 **junto con** el umbral de match — están acoplados: `_gallery_add()` solo corre después de un match ya confirmado (sim ≥ `SIMILARITY_THRESHOLD`), así que si el techo de diversidad se hubiera quedado en 0.85, todo match exitoso habría calificado automáticamente como "ángulo duplicado" y la galería nunca habría crecido más de 1 embedding. `PARTIAL_BODY_REID_THRESHOLD=0.64` (vistas parciales) no se tocó — pendiente de su propia calibración con datos específicos de vista parcial.
- **Solo cuerpo completo entra a la galería:** las vistas parciales (`PARTIAL_BODY_MIN_RATIO ≤ ratio < PARTIAL_BODY_MAX_RATIO`, ver más abajo) pueden **confirmar** una identidad existente (con el umbral más bajo `PARTIAL_BODY_REID_THRESHOLD=0.64`), pero nunca se agregan a la galería — ni en el primer match (`match_or_create(..., add_to_gallery=not partial)`) ni en el refresh periódico (`probes.py` solo llama `update_embedding()` cuando `not partial`). Antes de este cambio, `_gallery_add` corría sin condición en ambos puntos y una vista parcial/ocluida podía volverse miembro permanente de la galería, degradando el matching futuro contra esa identidad.
...
- `match_or_create(embedding, camera_id, threshold=None, create=True, track_id=None, add_to_gallery=True)` — retorna `(global_id, event_type, prev_camera_id, expired_ids)`. `expired_ids` son los `global_id`s que `_expire_stale()` acaba de olvidar en esta llamada — el caller (`probes.py`) los usa para limpiar `FaceRecognizer.forget()` y `_employee_by_global_id`. `track_id` es opcional y solo alimenta el log CSV, no afecta el matching. `add_to_gallery=False` permite confirmar un match sin agregar el embedding a la galería (usado para vistas parciales — ver "Re-ID entre Cámaras" arriba).
