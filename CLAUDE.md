...
**`app_video_testing.py`** (~410 líneas)
Igual que `app.py` pero para archivos MP4 locales. Usa `filesrc + decodebin` en lugar de `rtspsrc`. `decodebin` detecta el codec automáticamente (mp4v, h264, h265, etc.) — reemplazó `qtdemux + h264parse + nvv4l2decoder` que sólo soportaba H.264 y bloqueaba el playback de clips grabados con `mp4v`. Después de `decodebin`, `nvvideoconvert + capsfilter(NVMM, NV12)` llevan los frames al formato que requiere `nvstreammux`. Las dimensiones del streammux se detectan automáticamente con `cv2.VideoCapture` antes de construir el pipeline (evita distorsión en clips sub-stream o tileados). Acepta `--capabilities`, `--client`, `--input` y `--no-loop` por CLI. Lee el sector desde `/etc/nx_sector` para que los eventos de playback sean consistentes con el pipeline live. Llama `start_workers()` tras `PLAYING` (workers async operan durante el playback).

**Sink según modo:**
- **QA mode (`NX_QA_ENABLED=true`):** Mismo pipeline dual-probe que `app.py` QA — `tiler(1×1 640×360) → fakesink`, Probe A en `caps_rgba src-pad`, Probe B en `tiler src-pad`. Inicializa `MjpegServer` (:8080) y `RecordingManager`. El dashboard Streamlit muestra el video con overlays e inferencia en la tab Grabaciones. El `docker-entrypoint.sh` lee las capabilities y el cliente desde `nx:qa:status` en Redis y los pasa como `--capabilities` y `--client`.
- **Dev mode (sin QA):** `nvrtspoutsinkbin` en :8554 para visualizar con VLC u otro viewer RTSP.
...
- `_qa_overlay_probe`: **Probe B** (QA mode, post-tiler). Lee frame tileado 640×360. Primero envía el frame **crudo (sin overlays)** al `RecordingManager.push_tiled_frame()` si hay grabación activa — esto garantiza que `tiled.mp4` sea virgen. Luego dibuja bboxes in-place, empuja el frame con overlays a `tiled_frame_queue` (MJPEG), publica `nx:qa:detections`.
...
- `/viewer/<key>` — HTML mínimo con `<img>` + JS de reconexión automática: cuando el stream cae (ej. durante el reinicio del pipeline para modo playback), `img.onerror` reintenta cada 2 s con cache-buster hasta que el servidor vuelve a subir.
...
Carga y fusiona configuración desde 5 fuentes (prioridad: env vars > `/etc/nx_*` > `config.yaml` > `.env` > defaults). Define los 15 paquetes predefinidos (`PACKAGE_DEFINITIONS`), capacidades válidas, límites de NVDEC, y genera URLs RTSP interpolando el patrón del DVR. Retorna un `ClientConfig` dataclass. Campos configurables desde `config.yaml` (con defaults): `pgie_batch_size=0`, `pgie_interval=-1`, `sgie_interval=-1`, `reid_gallery_size=10` (máximo de embeddings por persona en `ReIdManager`; ajustar según número de cámaras del cliente), `recording_enabled=false` (activar grabación automática de clips cuando se detectan personas; funciona tanto en producción como en QA — en QA siempre graba independientemente de este campo). Campos de umbral PGIE sobreescribibles por cliente (todos con default -1 = usar valor del archivo): `pgie_topk` (máx detecciones por inferencia; subir si hay >20 personas en cámara), `pgie_nms_iou_threshold` (umbral IoU del NMS; bajar a 0.3 si personas juntas se colapsan en una detección), `pgie_pre_cluster_threshold` (confianza mínima pre-NMS; bajar a 0.2 para detectar personas ocluidas). Si alguno está seteado, `app.py` genera un config temporal en `/tmp/` vía `_apply_pgie_overrides()` sin modificar el archivo compartido del modelo. **Importante:** la función reescribe todas las rutas de archivo relativas (`onnx-file`, `model-engine-file`, `labelfile-path`, etc.) como rutas absolutas — sin esto, DeepStream las resolvería relativas a `/tmp/` y crashea con `Cannot access ONNX file '/tmp/...'` (ver ErrorHistory.md 2026-05-28). El engine TRT resultante se guarda en el directorio de modelos (volumen montado), no en `/tmp/`, evitando reconstrucciones de ~5 min en cada restart.
...
Worker thread para grabación automática de clips de video en QA mode. `notify_detection(N)` es llamado directamente desde los probes (sin Redis). Estado: IDLE → RECORDING → IDLE. Graba `tiled.mp4` (desde `push_tiled_frame()`, llamado por **Probe B directamente** antes de dibujar overlays — frame virgen sin bboxes) y `<camera_id>.mp4` full-res (desde `push_camera_frame()`, llamado por Probe A). Cooldown: 10 s sin detecciones. Duración mínima: 5 s (clips más cortos se descartan). Duración máxima: 5 min. Auto-prune: elimina clips más antiguos cuando total > 10 GB. Publica `nx:qa:recording_active` y `nx:qa:recording_info` a Redis. Directorio `/nx_tech/recordings/<YYYYMMDD_HHMMSS>/`: `thumbnail.jpg`, `metadata.json`, `tiled.mp4`, `<cam_id>.mp4`. **Importante:** `_max_people` y `_last_det_time` se resetean en `_reset_writers()` (NO en `_start()`). Sin el reset de `_last_det_time` el loop IDLE arrancaría un nuevo clip inmediatamente con `_max_people=0` (ver ErrorHistory.md 2026-06-02). `duration_s` en metadata = `frame_count / fps` (duración real del video file, no tiempo de reloj que incluiría el cooldown).
