...
- **Label en stream**: muestra `G#<global_id>` (ReID cross-cámara, 12 chars hex) si el embedding ya fue procesado, o `...` mientras espera. Los handlers appendean al prefijo existente: ej. `G#a3f92c1b8d04 | male_adult | 87%`. El `track_id` local ya no aparece en el stream. Face recognition agrega `| <nombre> NN%` al label solo cuando hay match confirmado (`identity_key != "Unknown"`), resuelto vía `_face_recognizer.get_display_name()`.
